 ...
 *
 * contractReference/userReference for a SINGLE mandate are generated by
 * CollexiaClient, confirmed against Collexia's own Postman pre-request
 * script (see that class's docblock). The legacy buildContractReference()
 * below is kept ONLY for split mandates: the script's contractReference is
 * time-based (gid+MMDD+HHmmss) with no per-order component, so several
 * splits placed in the same second in placeSplitMandate()'s loop would
 * collide -- an open conflict flagged for Collexia rather than silently
 * worked around; see the implementation report.
 *
...
class DebitOrderCollexiaController extends Controller
{
    ...
    public function syncStatus(string $id): void
    {
        ...
        $isSplit = (int) ($debitOrder['split_enabled'] ?? 0) === 1;
        $result = (new CollexiaMandateStatusSyncService())->syncDebitOrder($debitOrder);

        if (!$result['synced']) {
            Session::flash('error', $result['error']);
        } else {
            $statusNote = $result['mandate_status'] !== null
                ? ' Collexia reports ' . ($isSplit ? 'every split as ' : 'this mandate as ') . $result['mandate_status'] . '.'
                : '';
            Session::flash('success', ($isSplit
                ? 'Synced the latest status for every placed split transaction.'
                : 'Synced the latest mandate status.') . $statusNote);
        }
