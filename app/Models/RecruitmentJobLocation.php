...
{
    private const SORTABLE = ['name' => 'name', 'city' => 'city', 'status' => 'status', 'created_at' => 'created_at'];
    ...
    public function paginated(string $search = '', string $sort = 'name', string $dir = 'asc', int $page = 1, int $perPage = 10): array
    {
        ...
        $params = [];
        if ($search !== '') {
            $where = ' WHERE name LIKE ? OR city LIKE ? OR region LIKE ?';
            $like = '%' . $search . '%';
            array_push($params, $like, $like, $like);
        }
        $total = (int) $this->scalar("SELECT COUNT(*) FROM recruitment_job_locations" . $where, $params);
        ...
        $rows = $this->query(
            "SELECT * FROM recruitment_job_locations{$where} ORDER BY {$orderCol} {$orderDir} LIMIT {$perPage} OFFSET {$offset}",
            $params
        )->fetchAll();
