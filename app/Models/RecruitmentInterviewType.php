...
class RecruitmentInterviewType extends Model
{
    ...
    public function paginated(string $search = '', string $sort = 'name', string $dir = 'asc', int $page = 1, int $perPage = 10): array
    {
        ...
        $total = (int) $this->scalar("SELECT COUNT(*) FROM recruitment_interview_types" . $where, $params);
        ...
        $rows = $this->query(
            "SELECT * FROM recruitment_interview_types{$where} ORDER BY {$orderCol} {$orderDir} LIMIT {$perPage} OFFSET {$offset}",
            $params
        )->fetchAll();
