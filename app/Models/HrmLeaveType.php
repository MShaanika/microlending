...
class HrmLeaveType extends Model
{
    ...
    private const SORTABLE = ['name' => 'name', 'max_days_per_year' => 'max_days_per_year', 'status' => 'is_active'];
    ...
    public function paginated(string $search = '', string $sort = 'name', string $dir = 'asc', int $page = 1, int $perPage = 10): array
    {
        ...
        $total = (int) $this->scalar("SELECT COUNT(*) FROM hrm_leave_types" . $where, $params);
        ...
        $rows = $this->query(
            "SELECT * FROM hrm_leave_types{$where} ORDER BY {$orderCol} {$orderDir} LIMIT {$perPage} OFFSET {$offset}",
            $params
        )->fetchAll();
