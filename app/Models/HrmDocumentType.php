...
class HrmDocumentType extends Model
{
    ...
    private const SORTABLE = ['name' => 'name', 'required' => 'is_required'];
    ...
    public function paginated(string $search = '', string $sort = 'name', string $dir = 'asc', int $page = 1, int $perPage = 10): array
    {
        ...
        $total = (int) $this->scalar("SELECT COUNT(*) FROM hrm_document_types" . $where, $params);
        ...
        $rows = $this->query(
            "SELECT * FROM hrm_document_types{$where} ORDER BY {$orderCol} {$orderDir} LIMIT {$perPage} OFFSET {$offset}",
            $params
        )->fetchAll();
